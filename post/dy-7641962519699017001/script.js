...
document.querySelectorAll('pre').forEach(pre=>{
  ...
  btn.addEventListener('click', async ()=>{
    ...
    try{
      ...
      setTimeout(()=>{ btn.innerHTML = '<i class="fa-regular fa-copy"></i> 复制'; }, 1600);
    }
  });
  pre.appendChild(btn);
});
...
(function stars(){
  ...
  let avg = 0, total = 0;
  if(save){
    avg = save.avg; total = save.n;
    renderStars(save.v);
    count.textContent = `(${total} 人评分 · 平均 ${avg.toFixed(1)})`;
  }
  function renderStars(v){
    stars.forEach((s,i)=>{
      if(i < v){ s.classList.add('fa-solid'); s.classList.remove('fa-regular'); s.classList.add('active'); }
      else{ s.classList.remove('fa-solid'); s.classList.add('fa-regular'); s.classList.remove('active'); }
    });
  }
  stars.forEach(s=>{
...
document.getElementById('share-wechat')?.addEventListener('click', e=>{
  e.preventDefault(); alert('微信号: expectopatronus00');
});
document.getElementById('share-link')?.addEventListener('click', async ()=>{
  try{ await navigator.clipboard.writeText(location.href); alert('链接已复制'); }catch{ alert('请手动复制'); }
});
...
document.getElementById('comment-form')?.addEventListener('submit', e=>{
  ...
  if(!text){ document.getElementById('comment-content').focus(); return; }
