document.addEventListener('DOMContentLoaded', function () {
  ...
  function renderList(list) {
    ...
    if (!list.length) {
      grid.innerHTML =
        '<p style="grid-column:1/-1;text-align:center;color:var(--mute);padding:60px 0;">没有符合条件的文章</p>';
      return;
    }
    grid.innerHTML = list
      .map(function (a) {
        ...
        return (
          ...
          a.url +
          '" target="' +
          (a.url.startsWith('http') ? '_blank' : '_self') +
          '" rel="noopener">' +
  ...
  function renderTags(list) {
    ...
    box.innerHTML = tags
      .map(function (t) {
        return '<span data-tag="' + t[0] + '">#' + t[0] + ' (' + t[1] + ')</span>';
      })
      .join('');
    box.querySelectorAll('[data-tag]').forEach(function (el) {
      el.addEventListener('click', function () {
        setFilter('__tag__:' + el.dataset.tag);
      });
    });
  }

  function renderStats(list) {
    var a = document.getElementById('stat-art');
    var t = document.getElementById('stat-tag');
    var c = document.getElementById('stat-cat');
    ...
    var catMap = {};
    list.forEach(function (x) {
      catMap[x.category] = (catMap[x.category] || 0) + 1;
    });
    var topCat = Object.entries(catMap).sort(function (a, b) {
      return b[1] - a[1];
    })[0];
  ...
  function setFilter(v) {
    if (v && v.indexOf('__tag__:') === 0) {
      ...
      if (!v)
        catBtns.forEach(function (b) {
          if (!b.dataset.cat) b.classList.add('active');
  ...
  function applyFilters() {
    ...
    if (currentCat) {
      q = q.filter(function (a) {
        return a.category === currentCat;
      });
    }
    if (currentTag) {
      q = q.filter(function (a) {
        return (a.tags || []).indexOf(currentTag) > -1;
      });
    }
    if (currentQuery) {
      q = q.filter(function (a) {
        return (
          (a.title || '').toLowerCase().indexOf(currentQuery) > -1 ||
          (a.summary || '').toLowerCase().indexOf(currentQuery) > -1 ||
          (a.tags || []).some(function (t) {
            return ('' + t).toLowerCase().indexOf(currentQuery) > -1;
          })
        );
      });
    }
