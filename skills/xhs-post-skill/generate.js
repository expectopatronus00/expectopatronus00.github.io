#!/usr/bin/env node
/**
 * xhs-post-skill / generate.js
 * -----------------------------------------
 * 批量生成小红书搬运文章 + 自动更新 home.html / index.html
 *
 * 用法:
 *   node skills/xhs-post-skill/generate.js path/to/notes.json
 *
 * 输入文件 (JSON):
 *   参见 skills/xhs-post-skill/batch-template.json
 *
 * 输出:
 *   - post/xhs-{ID}/xhs-{ID}.html     (N个文章页)
...
if (args.length < 1) {
  console.error('用法: node generate.js <notes.json>');
  console.error('模板: skills/xhs-post-skill/batch-template.json');
  process.exit(1);
}
const inputPath = path.resolve(args[0]);
let notes;
try {
  notes = JSON.parse(read(inputPath));
} catch (e) {
  console.error('JSON 解析失败: ' + e.message);
  process.exit(1);
}
if (!Array.isArray(notes) || notes.length === 0) {
  console.error('notes.json 必须是非空数组');
  process.exit(1);
}
...
console.log('=== 开始批量生成 ' + notes.length + ' 篇文章 ===');
console.log();
...
for (let i = 0; i < notes.length; i++) {
  ...
  if (Array.isArray(v.tags) && v.tags.length > 0) {
    ...
    for (let m = 0; m < v.tags.length; m++) {
      tagItems.push('      <a href="../home/home.html?tag=' + v.tags[m] + '">#' + v.tags[m] + '</a>');
    }
    tagsHtml =
      '    <div class="tags-row">\n' +
      tagItems.join('\n') + '\n' +
  ...
  if (v.cta && v.cta.url) {
    ...
    ctaHtml =
      '    <div class="cta-box">\n' +
      '      <div class="content">\n' +
      '        <h3>' + (v.cta.title || '去小红书看原笔记') + '</h3>\n' +
      '        <p>' + (v.cta.desc || '点击跳转小红书原笔记页') + '</p>\n' +
  ...
  const footerMeta = v.footer_meta || '本站内容纯学习用途，非商用。图片版权归小红书原作者所有。';
  ...
  const html = template
    ...
    .replace('{{CONTENT_HTML}}', fullContent)
...
console.log('  git commit -m "feat: 新增' + generated.length + '篇小红书搬运文章"');
